...
There is no `BootNext`, `BootOrder`, completion marker, installer UUID,
automatic media re-entry handling, or kexec path.
...
10. installer poweroff;
11. boot of the installed system after installer media removal;
12. password login and sudo;
13. Btrfs `@root`, `@nix`, and `@persist`;
14. persistence and ephemeral-root reset across reboot.
...
- automatic firmware boot-order manipulation;
...
12. successful installation ends with sync, unmount, and poweroff;
13. first boot requires the user to remove/eject installer media and power on;
14. `.#update` remains the only normal dependency-update path;
15. deterministic checks and one networked E2E cover the supported contract.
